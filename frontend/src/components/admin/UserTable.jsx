...
function OnlineIndicator({ lastVisit }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      <span className={`w-2 h-2 rounded-full ${isOnline ? 'bg-emerald-400 animate-pulse-dot' : 'bg-slate-300'}`} />
  ...
  if (!users.length) {
    return (
      <div className="text-center py-16 text-slate-400">
        ...
        <p className="text-sm font-body">No users registered yet.</p>
      </div>
    );
  }
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-slate-50 border-b border-slate-200 text-left">
            ...
            <th className="px-4 py-3 font-display font-semibold text-navy text-xs uppercase tracking-wider hidden md:table-cell">Last Active</th>
